  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Early Detection — CariCare Grid" },
      {
        ...
        content: "Continuous vitals ingestion and trend-based deterioration alerts across the Caribbean.",
      },
...
function Detection() {
  ...
  const homeReadings = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("vitals")
        .select("id, patient_id, measured_at, systolic, diastolic, glucose_mmol, device, reported_by")
        .eq("source", "home")
  ...
  const sweep = useMutation({
    ...
    onSuccess: (n) => {
      toast.success(n > 0 ? `${n} new signal(s) raised` : "Sweep complete — no new deterioration found");
  ...
  return (
    <div className="space-y-5">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-primary">Early detection</p>
          <h1 className="font-display text-[26px] font-semibold tracking-tight">Deterioration, days before the crisis</h1>
          <p className="mt-1 max-w-3xl text-[13.5px] text-muted-foreground">
            Every reading — home cuff, glucometer, community screening or clinic device — is compared against that
            patient's own 40-day baseline. Drift raises a signal with a named action, long before an emergency
            presentation.
          </p>
        </div>
        <button
      ...
      <div className="grid gap-4 lg:grid-cols-[1.4fr_1fr]">
        <Panel>
          <PanelHeader title="Early-warning worklist" subtitle="Newest first · each signal names the next action" />
          {signals.isLoading ? (
            <Loading />
          ) : shown.length === 0 ? (
                  ...
                  <div key={s.id} className="px-5 py-4">
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-[12.5px] text-primary">{s.recommended_action}</p>
                      </div>
                      <div className="shrink-0 text-right">
                        <Pill className={SEVERITY_TONE[s.severity] ?? SEVERITY_TONE["watch"]!}>{s.severity}</Pill>
                        <p className="mt-1 text-[11px] text-muted-foreground">{timeAgo(s.detected_at)}</p>
                      </div>
                    </div>
            ...
            {(homeReadings.data ?? []).map((v) => {
              ...
              return (
                <div key={v.id} className="flex items-start justify-between gap-3 px-5 py-3">
                  ...
                  <span className="shrink-0 text-[11px] text-muted-foreground">{timeAgo(v.measured_at)}</span>
                </div>
              );
            })}
          </div>
