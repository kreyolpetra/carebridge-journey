/**
 * Screening campaigns and outreach, embedded by the surveillance screen.
 */
...
export function Prevention() {
  ...
  const byCampaign = useMemo(() => {
    const m = new Map<
      string,
      { total: number; sent: number; responded: number; booked: number; readings: number }
    >();
    for (const t of targets.data ?? []) {
      const cur = m.get(t.campaign_id) ?? {
        total: 0,
        sent: 0,
        responded: 0,
        booked: 0,
  ...
  const activeCampaign =
    (campaigns.data ?? []).find((c) => c.id === selected) ?? campaigns.data?.[0] ?? null;
    ...
    <div className="space-y-5">
      <header>
        ...
        <h1 className="font-display text-[26px] font-semibold tracking-tight">
          Reach patients before they crash
        </h1>
        <p className="mt-1 max-w-3xl text-[13.5px] text-muted-foreground">
          Risk scores are only useful if something happens. Build a cohort from the region's live
          data, send it down the care line in the patient's own language, and watch replies,
          readings and bookings come back.
        </p>
      </header>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Stat
          ...
          hint="Across all running campaigns"
          tone="signal"
        />
        ...
        <Stat
          ...
          hint="New data with no clinic visit"
          tone="low"
        />
        <Stat
          label="Booked into care"
          value={totals.booked}
          hint="Escalated to a consult"
          tone="critical"
        />
      </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <Pill className={STATUS_TONE[c.status] ?? STATUS_TONE["draft"]!}>
                        {c.status}
                      </Pill>
                    </div>
              ...
              <div className="space-y-3 px-5 py-4">
                  ...
                  <label className="block text-[12px] font-semibold uppercase tracking-wide text-muted-foreground">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setDraft({
                          ...draft,
                          rule: { ...draft.rule, condition: e.target.value || undefined },
                        })
                      }
                      className="mt-1 w-full rounded-lg border border-border bg-background px-3 py-2 text-[13.5px] font-normal normal-case tracking-normal text-foreground"
                    >
                  ...
                  <label className="block text-[12px] font-semibold uppercase tracking-wide text-muted-foreground">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setDraft({
                          ...draft,
                          rule: { ...draft.rule, island: e.target.value || undefined },
                        })
                      }
                      className="mt-1 w-full rounded-lg border border-border bg-background px-3 py-2 text-[13.5px] font-normal normal-case tracking-normal text-foreground"
                    >
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  <NumberField
                    ...
                    onChange={(v) =>
                      setDraft({ ...draft, rule: { ...draft.rule, daysSupplyMax: v } })
                    }
                  />
                  <NumberField
                    label="Adherence ≤ %"
                    ...
                    onChange={(v) =>
                      setDraft({ ...draft, rule: { ...draft.rule, adherenceMax: v } })
                    }
                  />
                </div>
                ...
                <label className="flex items-center gap-2 text-[13px]">
                  <input
                    ...
                    onChange={(e) =>
                      setDraft({ ...draft, rule: { ...draft.rule, ruralOnly: e.target.checked } })
                    }
                  />
                  Rural patients only
                </label>
                ...
                {preview.length > 0 ? (
                  <div className="rounded-lg border border-border bg-muted/50 px-3 py-2 text-[12px] text-muted-foreground">
                    First match:{" "}
                    <strong className="text-foreground">{preview[0]!.patient.full_name}</strong> —{" "}
                    {preview[0]!.reason}
                  </div>
                ) : null}
                ...
                >
                  ...
                  {launch.isPending
                    ? "Sending…"
                    : `Launch to ${Math.min(preview.length, 200)} patients`}
                </button>
              </div>
            </Panel>
          ) : null}
                    ...
                    return (
                      <div key={t.id} className="flex items-start justify-between gap-3 px-5 py-3">
                        <div className="min-w-0">
                          ...
                          {t.outcome ? (
                            <p className="text-[12px] text-primary">{t.outcome}</p>
                          ) : null}
                        </div>
